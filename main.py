...
def main():
    ...
    @client.command(["lock_command", "заморозить_команду"], condition=is_agent)
    def add_locked_command(data: Parameters):
        ...
        for command in content:
            if command in commands:
                ...
                reply(data, f"Команда {command} заморожена")
            else:
                reply(data, f"Нет команды {command}")
...
''')
    ...
    def change_icon(data: Parameters, image_link: str):
        h = httplib2.Http('.cache')
        response, content = h.request(image_link)
        with open(f'icon\\icon.jpg', 'wb') as f:
            f.write(content)
        try:
            with open(file=f"icon\\icon.jpg", mode="rb") as file_img:
